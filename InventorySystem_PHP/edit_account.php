...
//update user image
  ...
  if($photo->process_user($user_id)){
    $session->msg('s','foto telah diunggah.');
  ...
  if(isset($_POST['update'])){
    ...
    $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            $session->msg('s',"Akun di perbarui ");
            redirect('edit_account.php', false);
          } else {
            $session->msg('d',' Maaf gagal memperbarui!');
            redirect('edit_account.php', false);
          }
    } else {
      ...
      <div class="panel-body">
          <form method="post" action="edit_account.php?id=<?php echo (int)$user['id'];?>" class="clearfix">
            ...
            <div class="form-group clearfix">
                    <a href="change_password.php" title="change password" class="btn btn-danger pull-right">Perbarui Password</a>
